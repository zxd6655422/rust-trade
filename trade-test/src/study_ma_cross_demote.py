"""短均线交叉降级 vs 回撤降级 对比（在三段式 A11 段3 框架下）。

段3（大单 MA480 长拿）的降级信号对比：
  - drawdown：回撤 demote_pct=10% 降回 MA192（A9/A11 原始）
  - ma_cross：MA48 跌破 MA96（做多死叉）降回 MA192（结构性趋势减弱，更早）
  - both：回撤 或 短均线交叉，任一触发降级

固定 switch1/switch2 用 A11 最优（BTC 6/12、ETH 4/15、SOL 4/20）。
输出：feature_report/ma_cross_demote_report.md
"""
...
def backtest(symbol, params, bars, pre, switch1, switch2, demote_mode,
             demote_pct=10.0, confirm=10, y0=None, y1=None):
    ...
    ma192s = pre["ma192"]
    ma480s = pre["ma480"]
    ma48s = sma_series(closes, 48)
    ma96s = sma_series(closes, 96)
    prefix = pre["prefix"]
    ...
    for i in range(n):
        ...
        if pos is not None:
            ...
            # 短均线交叉检测（做多死叉 / 做空金叉）
            ma_cross = False
            if ma48s[i] is not None and ma96s[i] is not None and ma48s[i-1] is not None and ma96s[i-1] is not None:
                if side == "LONG":
                    ma_cross = ma48s[i-1] >= ma96s[i-1] and ma48s[i] < ma96s[i]  # 死叉
                else:
                    ma_cross = ma48s[i-1] <= ma96s[i-1] and ma48s[i] > ma96s[i]  # 金叉

            if pos.get("demoted"):
                use_ma288_stop, tp = False, 192
            elif pos["max_profit"] >= switch2:
                profit_decline = pos["max_profit"] - pnl
                dd_trigger = profit_decline >= demote_pct and pnl < pos["max_profit"] - demote_pct
                if demote_mode == "drawdown":
                    trigger = dd_trigger
                elif demote_mode == "ma_cross":
                    trigger = ma_cross
                elif demote_mode == "both":
                    trigger = dd_trigger or ma_cross
                else:
                    trigger = False
                if trigger:
                    pos["demoted"] = True
                ...
                if pos["max_profit"] >= 4.0 and pos["max_profit"] - pnl >= 1.5:
...
def main() -> int:
    md = []
    add = md.append
    add("# 短均线交叉降级 vs 回撤降级（三段式段3）")
    add("")
    add("- drawdown=回撤10%降级；ma_cross=MA48×MA96交叉降级；both=任一触发。")
    add("")

    PARAMS = {"BTCUSDT": (6.0, 12.0), "ETHUSDT": (4.0, 15.0), "SOLUSDT": (4.0, 20.0)}
    ...
    for coin in ["BTCUSDT", "ETHUSDT", "SOLUSDT"]:
        ...
        add(f"## {coin}（switch1={s1}%/switch2={s2}%）")
        add("")
        add("| 降级方式 | 全样本复利 | 验证段复利 | 验证段基线 | 验证段胜负 |")
        add("|---|---|---|---|---|")
        for mode in ["drawdown", "ma_cross", "both"]:
            full = comp([t["ret_pct"] for t in backtest(coin, params, bars, pre, s1, s2, mode)])
            val = comp([t["ret_pct"] for t in backtest(coin, params, bars, pre, s1, s2, mode, y0=mid+1, y1=y1)])
            val_base = comp([t["ret_pct"] for t in backtest(coin, params, bars, pre, 999, 999, "drawdown", y0=mid+1, y1=y1)])
            win = "✅" if val > val_base else "❌"
            add(f"| {mode} | {full:+.1f}% | {val:+.1f}% | {val_base:+.1f}% | {win} |")
        add("")

    out = os.path.join(os.path.dirname(os.path.abspath(__file__)), "feature_report", "ma_cross_demote_report.md")
