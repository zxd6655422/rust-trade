...
| 模块 | 状态 | 说明 |
|------|------|------|
| 统一 Timeframe 枚举 | ✅ 已完成 | 移除非标准框架（3m/45m/6h/8h/12h），保留10个标准框架 |
| Redis 连接池改造 | ✅ 已完成 | ConnectionManager，全异步，自动重连 |
| 数据完整性验证 | ✅ 已完成 | 采集层（时间戳验证）+ 策略层（数据量+新鲜度验证） |
| 市场情绪数据采集 | ✅ 已完成 | 资金费率/持仓量/多空比，PostgreSQL + Redis |
| 订单簿/大单实时获取 | ✅ 已完成 | 策略执行时实时调用 Binance API |
| 多时间框架回填 | ✅ 已完成 | 支持 5m/15m/30m/1h/2h/4h/1d/3d/1w |
| Redis 高TF缓存刷新 | ✅ 已完成 | 每30分钟从DB刷新，Redis重启后立即刷新 |
| 代码质量审查 | ✅ 已完成 | 修复18个问题（4严重+7中等+7轻微） |
...
| 模块 | 状态 | 说明 |
|------|------|------|
| 统一数据结构 | ✅ 已完成 | AccountSnapshot / AssetBalance / PositionInfo |
| AccountProvider trait | ✅ 已完成 | 统一接口，支持多交易所 |
| Binance AccountProvider | ✅ 已完成 | 合约（/fapi/v2/account）+ 现货（/api/v3/account） |
| OKX AccountProvider | ✅ 已完成 | 合约（/api/v5/account/balance?instType=SWAP）+ 现货 |
| 数据库表设计 | ✅ 已完成 | account_snapshot_v2 / asset_balance / position_snapshot |
| AccountSync 重构 | ✅ 已完成 | 支持 Binance + OKX 双交易所同步 |
...
| 任务 | 优先级 | 说明 |
|------|--------|------|
| 账户快照测试 | P0 | 验证 Binance/OKX 账户同步是否正常 |
| 市场情绪数据测试 | P0 | 验证资金费率/持仓量/多空比采集 |
| Redis 缓存测试 | P0 | 验证重启后数据恢复、高TF缓存刷新 |
| 数据完整性验证测试 | P0 | 验证策略层在数据不足时正确跳过 |

#### P1: 策略集成（下周）

| 任务 | 优先级 | 说明 |
|------|--------|------|
| 策略引擎集成账户数据 | P1 | 策略执行时读取账户余额和持仓信息 |
| 策略引擎集成市场情绪 | P1 | 使用资金费率/持仓量/多空比辅助决策 |
| 动态止损止盈 | P1 | 基于 ATR 和账户风险动态调整止损止盈 |
| 仓位管理优化 | P1 | 基于账户总权益和风险比例计算仓位大小 |

#### P2: 监控告警（两周内）

| 任务 | 优先级 | 说明 |
|------|--------|------|
| 账户余额监控 | P2 | 余额变化超过阈值时告警 |
| 持仓风险监控 | P2 | 保证金率接近强平线时告警 |
| 市场情绪监控 | P2 | 资金费率异常时告警 |
| 数据采集监控 | P2 | 采集失败连续N次时告警 |

#### P3: 优化改进（三周内）

| 任务 | 优先级 | 说明 |
|------|--------|------|
| Redis 连接池优化 | P3 | 复用连接，减少 TCP 握手开销 |
| API 限速统一 | P3 | 统一 Binance/OKX 的限速策略 |
| 数据压缩 | P3 | Redis 存储使用更紧凑的格式 |
| 历史数据归档 | P3 | 定期归档旧数据到 Parquet |
