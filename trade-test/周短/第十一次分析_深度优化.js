 ...
 *
 * 1. 30m + 固定止损 → 如何拿住更大利润
 * 2. 30m + MA288止损 → 如何拿住更大利润
 * 3. 5m + MA288止损 → 结合布林带优化
 */
...
function addIndicators(df, prefix = '') {
  ...
  const bbPos = new Array(df.length).fill(null); // 价格在布林带中的位置 (0-100%)
...
function run30mFixedStop(df, config) {
  const {
    stopLossPct = 2.0,
    stopMode = 'fixed',        // 'fixed' 或 'ma288'
    // 止盈模式
    tpMode = 'trailing',      // trailing, bb, bb_trailing, ma48, combo
...
function run5mBB(df, config) {
  const {
    ...
    filter30m = true,
    // BB入场过滤
    bbEntryEnabled = false,     // 只在BB特定位置入场
    bbEntryLongMax = 50,       // 做多时BB位置上限
    bbEntryShortMin = 50,      // 做空时BB位置下限
    // 止盈
    tpMode = 'trailing',
...
// ============================================================
console.log("\n" + "=".repeat(70));
console.log("【最终对比: 三个策略优化后】");
console.log("=".repeat(70));

console.log(`
策略                    | 交易数 | 胜率   | 总收益   | 平均收益 | 最大盈  | 盈亏比
------------------------|--------|--------|----------|----------|---------|-------
30m+固定止损(之前)       |     52 |  28.8% | + 54.39% | + 1.046% |  20.60% |   3.92
30m+固定止损(优化后)     | ${String(best30mFixed.tradeCount).padStart(6)} | ${best30mFixed.winRate.toFixed(1).padStart(5)}% | ${(best30mFixed.totalPnL >= 0 ? '+' : '') + best30mFixed.totalPnL.toFixed(2).padStart(7)}% | ${(best30mFixed.avgPnL >= 0 ? '+' : '') + best30mFixed.avgPnL.toFixed(3).padStart(7)}% | ${best30mFixed.maxWin.toFixed(2).padStart(7)}% | ${best30mFixed.profitFactor.toFixed(2).padStart(6)}

30m+MA288止损(之前)      |    402 |  15.2% | + 42.79% | + 0.106% |  25.24% |  10.75
30m+MA288止损(优化后)    | ${String(best30mMA288.tradeCount).padStart(6)} | ${best30mMA288.winRate.toFixed(1).padStart(5)}% | ${(best30mMA288.totalPnL >= 0 ? '+' : '') + best30mMA288.totalPnL.toFixed(2).padStart(7)}% | ${(best30mMA288.avgPnL >= 0 ? '+' : '') + best30mMA288.avgPnL.toFixed(3).padStart(7)}% | ${best30mMA288.maxWin.toFixed(2).padStart(7)}% | ${best30mMA288.profitFactor.toFixed(2).padStart(6)}

5m+MA288止损(之前)       |    416 |  16.8% | +  8.86% | + 0.021% |   6.59% |   1.09
5m+MA288止损(优化后)     | ${String(best5mBB.tradeCount).padStart(6)} | ${best5mBB.winRate.toFixed(1).padStart(5)}% | ${(best5mBB.totalPnL >= 0 ? '+' : '') + best5mBB.totalPnL.toFixed(2).padStart(7)}% | ${(best5mBB.avgPnL >= 0 ? '+' : '') + best5mBB.avgPnL.toFixed(3).padStart(7)}% | ${best5mBB.maxWin.toFixed(2).padStart(7)}% | ${best5mBB.profitFactor.toFixed(2).padStart(6)}
`);

console.log("第十一次分析完成！");
