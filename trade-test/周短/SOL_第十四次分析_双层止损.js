/**
 * SOL 第十四次分析: 双层止损 (硬止损 + MA288趋势止损)
 *
 * 基于第十三次SOL最优结果:
 * - 30m入场 + MA288止损 + 5m+30m双扩散过滤
 * - slope=2, bbw=0, vol=0.5 (SOL专用参数)
 * - 移动止盈(5+5)
 *
 * 第十四次优化:
...
console.log("SOL 第十四次分析: 双层止损 (硬止损 + MA288趋势止损)");
console.log("=".repeat(70));
...
function runStrategy(df, config) {
  const {
    ...
    slopeThreshold = 2,       // SOL专用: 更宽松
    bbwThreshold = 0,         // SOL专用: 不用bbw
    volThreshold = 0.5,       // SOL专用
    use5mExpanding = true,
    use30mExpanding = true,   // SOL最优: 5m+30m双扩散
    minAngle5m = 0,
    entryTimeframe = '30m',
  } = config;
...
// ============================================================
console.log("\n" + "=".repeat(70));
console.log("【最终对比: SOL 第十四次(双层止损) vs 第十三次】");
console.log("=".repeat(70));
...
-----------------------------|--------|--------|----------|---------|-------
第十三次(5m+30m双扩散)        | 参考   | 参考   | +70.77%  |  参考   |  参考
