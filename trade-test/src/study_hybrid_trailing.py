...
两段式止盈：
  段1（小单，max_profit < switch_at）：移动止盈 activate_small + callback_small（高敏锁利，同 A1）
  段2（大单，max_profit >= switch_at）：切 MA480 长拿（关 MA288）+ 衰竭降级 demote_pct 降回 MA192

扫描 switch_at × activate_small × callback_small，找「既锁小单又拿大单」的最优。
口径：slow=480 + vol过滤 + 硬止损→MA288止损→混合止盈→趋势反转。

输出：feature_report/hybrid_trailing_report.md
"""
...
def backtest_hybrid(symbol, params, bars, pre, switch_at, activate_small, callback_small,
        ...
        if pos is not None:
            ...
            if pos.get("demoted"):
                ...
                if profit_decline >= demote_pct and pnl < pos["max_profit"] - demote_pct:
                    pos["demoted"] = True
                    use_ma288_stop, tp = True, 192  # 降级瞬间恢复 MA288 + MA192
                else:
...
def main() -> int:
    ...
    add("# 混合止盈：高敏锁小利 + 拿住大单")
    add("")
    add("- 段1（<switch_at）：移动止盈 activate_small+callback_small；段2（≥switch_at）：MA480长拿+衰竭降级10%。")
    add("")

    SWITCH = [8.0, 10.0, 12.0, 15.0, 20.0]
    ACT_SMALL = [3.0, 4.0, 5.0]
    CB_SMALL = [0.5, 1.0, 1.5]

    for coin in ["BTCUSDT", "ETHUSDT", "SOLUSDT"]:
        params = dc.SYMBOL_PARAMS[coin]
        bars = load_klines_30m(coin)
        pre = precompute(bars)
        base = comp([t["ret_pct"] for t in backtest_hybrid(coin, params, bars, pre, 999, 4, 1)])
        # 基线 = 纯 A1（switch_at 极大，永不升级）
        add(f"## {coin}  （基线 A1 复利 {base:+.1f}%）")
        add("")
        grid = []
        for sw, as_, cb in product(SWITCH, ACT_SMALL, CB_SMALL):
            trades = backtest_hybrid(coin, params, bars, pre, sw, as_, cb)
            c = comp([t["ret_pct"] for t in trades])
            grid.append((sw, as_, cb, c, len(trades)))
        grid.sort(key=lambda x: -x[3])
        add("### Top 15（按复利）")
        add("")
        add("| switch_at | activate_small | callback_small | 复利 | 笔数 |")
        add("|---|---|---|---|---|")
        for sw, as_, cb, c, n in grid[:15]:
            add(f"| {sw}% | {as_}% | {cb}% | {c:+.1f}% | {n} |")
        add("")
        ...
        mid = (y0 + y1) // 2
        best = grid[0]
        sw, as_, cb = best[0], best[1], best[2]
        val = comp([t["ret_pct"] for t in backtest_hybrid(coin, params, bars, pre, sw, as_, cb, y0=mid+1, y1=y1)])
        val_base = comp([t["ret_pct"] for t in backtest_hybrid(coin, params, bars, pre, 999, 4, 1, y0=mid+1, y1=y1)])
        add(f"**时间切分（训练 {y0}-{mid} → 验证 {mid+1}-{y1}）：最优 switch={sw}%/act={as_}%/cb={cb}% → 验证段 {val:+.1f}% vs 基线 {val_base:+.1f}%**")
        add("")

    out = os.path.join(os.path.dirname(os.path.abspath(__file__)), "feature_report", "hybrid_trailing_report.md")
