...
| `primary_timeframe` | 30m | 主要分析周期(趋势判断) |
| `entry_timeframe` | 30m | 入场K线周期(30m或5m) |
...
- **ETH**: 强烈推荐启用5m入场+扩散，收益从+29.95%提升到+69.44% (+132%)
- **SOL**: 强烈推荐启用30m入场+扩散，收益从+41.47%提升到+84.45% (+104%)
- **BTC**: 不建议启用，收益从+42.79%下降到+40.46% (-5%)

### 配置示例

**ETH配置 (5m入场 + 5m扩散 + 夹角>1°):**
```json
{
    "fast_ma_period": 288,
    "slow_ma_period": 488,
