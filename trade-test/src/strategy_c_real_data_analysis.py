...
class StrategyCAnalyzer:
    ...
    def calculate_statistics(self, trades: List[Dict], initial_capital: float = 10000.0) -> Dict:
        ...
        if not trades:
            return {
                ...
                'profit_factor': 0,
                'avg_hold_bars': 0,
                'avg_compression_bars': 0,
                'avg_boll_width_pct': 0
            }
        ...
        return {
            ...
            'avg_compression_bars': avg_compression_bars,
            'avg_boll_width_pct': avg_boll_width_pct
        }
    
    def analyze_by_year(self, trades: List[Dict], df: pd.DataFrame) -> Dict:
        """按年度分析交易表现"""
        if not trades:
            return {}
        ...
        for year in sorted(trades_df['year'].unique()):
            ...
            # 计算平均持仓bars
            avg_hold_bars = year_trades['hold_bars'].mean()
            
            # 计算平均压缩bars
            avg_compression_bars = year_trades['compression_bars_at_entry'].mean()
    ...
    def analyze_symbol(self, symbol: str, params: dict) -> Dict:
        ...
        # 按年度分析
        yearly_stats = self.analyze_by_year(trades, df)
    ...
    def generate_report(self, results: Dict, params: dict, output_dir: str):
        """生成详细报告"""
        report_file = os.path.join(output_dir, "strategy_c_real_data_analysis.md")
        
        with open(report_file, 'w', encoding='utf-8') as f:
            f.write("# 方案C：BOLL压缩突破策略 - 真实数据分析报告\n\n")
            ...
            # 全币种汇总
            f.write("## 二、全币种汇总\n\n")
            f.write("| 币种 | 交易数 | 胜率 | 简单收益 | 复利收益 | 最大回撤 | 盈亏比 | 平均持仓bars | 平均压缩bars |\n")
            f.write("|------|--------|------|----------|----------|----------|--------|--------------|--------------|\n")
            ...
            for symbol, result in results.items():
                if 'error' in result:
                    ...
                    f.write(f"| {symbol} | {stats['total_trades']} | {stats['win_rate']:.1f}% | "
                           ...
                           f"{stats['avg_hold_bars']:.1f} | {stats['avg_compression_bars']:.1f} |\n")
            ...
            for symbol, result in results.items():
                ...
                if yearly_stats:
                    f.write("| 年份 | 交易数 | 胜率 | 简单收益 | 复利收益 | 最大回撤 | 盈亏比 | 平均持仓bars | 离场原因 |\n")
                    f.write("|------|--------|------|----------|----------|----------|--------|--------------|----------|\n")
                    ...
                    for year in sorted(yearly_stats.keys()):
                        ...
                        f.write(f"| {year} | {stats['total_trades']} | {stats['win_rate']:.1f}% | "
                               f"{stats['total_pnl_pct']:.2f}% | {stats['compound_return_pct']:.2f}% | "
                               f"{stats['max_drawdown_pct']:.2f}% | {stats['profit_factor']:.2f} | "
                               f"{stats['avg_hold_bars']:.1f} | {exit_reasons_str} |\n")
            ...
            f.write("## 五、相关文件\n\n")
            f.write("- 策略脚本: `src/strategy_c_real_data_analysis.py`\n")
            f.write("- 本报告: `src/feature_report/strategy_c_real_data_analysis.md`\n")
...
def main():
    ...
    # 初始化分析器
    analyzer = StrategyCAnalyzer(data_dir)
    ...
    # 保存详细结果到JSON
    output_file = os.path.join(output_dir, "strategy_c_real_data_results.json")
