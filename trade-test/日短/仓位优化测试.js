...
function backtest(data, config) {
    ...
    return { trades, finalCapital: capital, totalCommission };
}
...
function main() {
    ...
    // 测试不同的仓位比例和杠杆组合
    const testCases = [
        { position: 0.1, leverage: 1, name: '10%仓位 1倍杠杆' },
        { position: 0.2, leverage: 1, name: '20%仓位 1倍杠杆' },
        { position: 0.3, leverage: 1, name: '30%仓位 1倍杠杆' },
        { position: 0.5, leverage: 1, name: '50%仓位 1倍杠杆' },
        { position: 1.0, leverage: 1, name: '100%仓位 1倍杠杆' },
        { position: 0.1, leverage: 2, name: '10%仓位 2倍杠杆' },
        { position: 0.1, leverage: 3, name: '10%仓位 3倍杠杆' },
        { position: 0.2, leverage: 2, name: '20%仓位 2倍杠杆' },
        { position: 0.2, leverage: 3, name: '20%仓位 3倍杠杆' },
        { position: 0.3, leverage: 2, name: '30%仓位 2倍杠杆' },
        { position: 0.5, leverage: 2, name: '50%仓位 2倍杠杆' },
    ];
    ...
    testCases.forEach(tc => {
        const config = {
            ...CONFIG,
            position_size_percent: tc.position,
            leverage: tc.leverage,
        };

        const result = backtest(data, config);
        const totalReturn = ((result.finalCapital / CONFIG.initial_capital) - 1) * 100;
        const netProfit = result.finalCapital - CONFIG.initial_capital;

        // 计算最大回撤
        let capital = CONFIG.initial_capital;
        let peak = capital, maxDrawdown = 0;
        const tempTrades = result.trades;
        ...
        results.push({
            ...
            winRate: result.trades.filter(t => t.netPnl > 0).length / result.trades.length,
        });
    });
    ...
    results.forEach(r => {
        console.log(
            r.name.padEnd(25) +
            `${r.finalCapital.toFixed(2)}`.padEnd(12) +
    ...
    console.log('🏆 最佳配置');
    console.log('='.repeat(90));
    console.log(`策略: ${best.name}`);
    console.log(`最终资金: ${best.finalCapital.toFixed(2)} USDT`);
    ...
    console.log('='.repeat(90));
    console.log('\n手续费影响分析:');
    console.log(`  手续费占净利润比例: ${best.netProfit > 0 ? (best.totalCommission / best.netProfit * 100).toFixed(1) : 'N/A'}%`);
    console.log('\n建议:');
    if (best.netProfit > 0) {
        console.log('  ✅ 策略有盈利，可以实盘测试');
        console.log(`  ✅ 推荐使用: ${best.name}`);
    } else {
        console.log('  ⚠️ 策略在当前参数下亏损');
        console.log('  ⚠️ 建议优化策略或等待更好行情');
    }
}
