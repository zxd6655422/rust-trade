...
function backtest(data, config, options = {}) {
    ...
    for (let i = startIdx; i < n; i++) {
        ...
        if (position !== 0) {
            ...
            // 优化方案3: 最小持仓时间
            if (options.minHoldBars && holdBars < options.minHoldBars) {
                // 不离场，继续持有
            } else {
                // 布林轨反弹离场
...
function main() {
    ...
    console.log('优化方案3: 增加最小持仓时间测试');
    console.log('='.repeat(80));
    console.log(`数据: ${data.length}条K线\n`);

    // 测试原版
    const original = backtest(data, CONFIG, { filterFakeSignals: false, consecutiveCross: false, minHoldBars: 0 });
    const originalStats = analyze(original, '原版策略');

    // 测试优化方案3
    const optimized = backtest(data, CONFIG, { filterFakeSignals: false, consecutiveCross: false, minHoldBars: 6 });
    const optimizedStats = analyze(optimized, '优化方案3');
