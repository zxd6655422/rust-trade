"""方向2：延迟入场确认 —— 用后续走势判断是否入场（BTC/ETH）。

MA288/MA480 回踩信号不变（趋势 + 收盘穿越 MA288），但触发后不立即入场，
而是观察后续 N 根 bar 的走势，满足「浮盈 >= X%」才确认入场；否则放弃该信号。

delay_n=None 表示立即入场（基线，与 backtest.py 完全一致）。
输出：feature_report/delayed_entry_report.md
"""
...
def _open_pos(side, entry, idx, hard_stop_pct):
    if side == "LONG":
        hard_stop = entry * (1.0 - hard_stop_pct / 100.0)
    else:
        hard_stop = entry * (1.0 + hard_stop_pct / 100.0)
    return {"side": side, "entry_price": entry, "entry_idx": idx,
            "hard_stop_price": hard_stop, "max_profit": 0.0}


def backtest(symbol, params, bars, delay_n: Optional[int], min_gain: float):
    ...
    trades = []
    pos = None
    pending = None  # {"side", "ref_price", "trigger_idx"}
    ...
    for i in range(n):
        ...
        if pending is not None:
            ...
            if trend_broke or price_broke:
                ...
                if gain >= min_gain:
                    pos = _open_pos(p["side"], close, i, params.hard_stop_pct)
                pending = None
        ...
        if pos is None and pending is None and fast_ma is not None and slow_ma is not None and prev_fast_ma is not None:
            ...
            if sig:
                if delay_n is None:
                    pos = _open_pos(sig, close, i, params.hard_stop_pct)
                else:
...
def main() -> int:
    md: List[str] = []
    add = md.append
    add("# 方向2：延迟入场确认（后续走势判断入场）")
    add("")
    add("- 信号不变（趋势 + 收盘穿越 MA288）；触发后延迟 N 根 bar，若第 N 根浮盈 >= X% 才入场，否则放弃。")
    add("- 入场价 = 确认时收盘价。delay=None 为立即入场基线。")
    add("")

    GRID = [(1, 0.0), (2, 0.0), (3, 0.0), (2, 0.3), (3, 0.3), (3, 0.5), (5, 0.5), (5, 1.0)]
    for coin in ["BTCUSDT", "ETHUSDT"]:
        params = dc.SYMBOL_PARAMS[coin]
        bars = load_klines_30m(coin)
        add(f"## {coin}")
        add("")
        add("| 延迟N根 | 浮盈≥X | 交易数 | 胜率 | 简单收益 | 复利收益 | 平均亏损 | 平均盈利 |")
        add("|---|---|---|---|---|---|---|---|")
        base = backtest(coin, params, bars, None, 0.0)
        m = metrics(base)
        add(f"| 0(立即) | — | {m['n']} | {m['wr']:.1f}% | {m['simple']:+.2f}% | {m['comp']:+.2f}% | {m['avg_loss']:+.2f}% | {m['avg_win']:+.2f}% |")
        for dn, mg in GRID:
            ts = backtest(coin, params, bars, dn, mg)
            m = metrics(ts)
            add(f"| {dn} | ≥{mg}% | {m['n']} | {m['wr']:.1f}% | {m['simple']:+.2f}% | {m['comp']:+.2f}% | {m['avg_loss']:+.2f}% | {m['avg_win']:+.2f}% |")
        add("")

    out = os.path.join(os.path.dirname(os.path.abspath(__file__)), "feature_report", "delayed_entry_report.md")
