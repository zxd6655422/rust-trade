"""方向4：回踩限价单 —— 信号触发后挂限价单等回踩，不追高。

- 信号触发（趋势 + 收盘穿越 MA288），记录信号价 ref_price。
- 挂限价单：做多限价 = ref_price * (1 - discount/100)（discount 为回踩折扣，0=信号价）。
- 成交：后续 bar.low <= 限价（做多）即成交，入场价 = 限价价。
- 撤单：趋势反转（MA288/MA480 交叉）或（可选）价格跌破 MA288 时撤单。
- 直接涨、不回踩的单子 → 错过（不成交）。

输出：feature_report/pullback_limit_report.md
"""
...
def backtest_pullback(symbol, params, bars, discount: float, reject_ma288: bool):
    ...
    for i in range(n):
        ...
        if pending is not None:
            ...
            if trend_broke or (reject_ma288 and price_broke):
                pending = None
                missed += 1
            else:
                if p["side"] == "LONG" and bar.low <= p["limit_price"]:
                    pos = open_pos(p["side"], p["limit_price"], i)
                    pending = None
        ...
        if pos is None and pending is None and fast_ma is not None and slow_ma is not None and prev_fast_ma is not None:
            ...
            if sig:
                signal_total += 1
                limit = close * (1.0 - discount / 100.0) if sig == "LONG" else close * (1.0 + discount / 100.0)
                pending = {"side": sig, "ref_price": close, "limit_price": limit, "trigger_idx": i}
    ...
    return trades, missed, signal_total
...
def main() -> int:
    ...
    add("# 方向4：回踩限价单（不追高）")
    add("")
    add("- 信号触发后挂限价单：做多限价 = 信号价*(1-discount%)，回踩触及才成交，直接涨则错过。")
    add("- 撤单：trend=趋势反转撤单；ma288=趋势反转或跌破MA288撤单。")
    add("")
    ...
    for coin in ["BTCUSDT", "ETHUSDT"]:
        ...
        add(f"## {coin}")
        add("")
        add("| 回踩折扣 | 撤单条件 | 交易数 | 胜率 | 简单收益 | 复利收益 | 平均亏损 | 平均盈利 | 错过信号 |")
        add("|---|---|---|---|---|---|---|---|---|")
        for disc in (0.0, 0.1, 0.3, 0.5, 0.8, 1.0):
            for rm, rm_name in ((False, "trend"), (True, "ma288")):
                ts, missed, sig_total = backtest_pullback(coin, params, bars, disc, rm)
                m = metrics(ts)
                add(f"| {disc}% | {rm_name} | {m['n']} | {m['wr']:.1f}% | {m['simple']:+.2f}% | {m['comp']:+.2f}% | {m['avg_loss']:+.2f}% | {m['avg_win']:+.2f}% | {missed} |")
        add("")

    out = os.path.join(os.path.dirname(os.path.abspath(__file__)), "feature_report", "pullback_limit_report.md")
