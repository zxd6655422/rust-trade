...
function backtest(data, config, options = {}) {
    ...
    let crossBelowCount = 0, crossAboveCount = 0; // 连续穿越中轨计数
...
function main() {
    ...
    console.log('优化方案2: 优化离场逻辑测试');
    console.log('='.repeat(80));
    console.log(`数据: ${data.length}条K线\n`);

    // 测试原版
    const original = backtest(data, CONFIG, { filterFakeSignals: false, consecutiveCross: false });
    const originalStats = analyze(original, '原版策略');

    // 测试优化方案2
    const optimized = backtest(data, CONFIG, { filterFakeSignals: false, consecutiveCross: true });
    const optimizedStats = analyze(optimized, '优化方案2');
